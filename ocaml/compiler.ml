...
let _ =
  ...
  let s = "%list  := {'01', '04', '02', '03', '02', '03'} \\ ({'03'} + {'42', 'pouet'})" in
  let lines = String.nsplit s "\n" in
  let lexbuf = Lexing.from_string s in
  try
    let parsed = Parser.main Lexer.token lexbuf in
    print_endline (dump parsed); apply_assignments parsed; print_endline (dump (StringSet.elements (Hashtbl.find heap "%list")))
  with
  | Parser.Error ->
